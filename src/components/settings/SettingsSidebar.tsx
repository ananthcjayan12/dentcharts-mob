...
const SettingsSidebar: React.FC = () => {
    ...
    return (
        <aside className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-gray-200 flex-shrink-0 z-10 sticky top-0 md:static">
            {/* Desktop Header */}
            <div className="hidden md:block p-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800">Settings</h2>
            </div>

            {/* Navigation List - Responsive (Vertical Scroll on Mobile, Vertical Tab on Desktop) */}
            <nav className="flex flex-row md:flex-col p-2 md:p-4 space-x-2 md:space-x-0 md:space-y-1 overflow-x-auto md:overflow-visible no-scrollbar">
                {navItems.map((item) => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        className={({ isActive }) =>
                            `flex items-center px-3 md:px-4 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap md:whitespace-normal flex-shrink-0 md:flex-shrink-1 ${isActive
